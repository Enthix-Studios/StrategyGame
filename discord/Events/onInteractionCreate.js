	...
	if (interaction.isCommand()) {
		for (let [k, v] of imports.ModuleImports.entries()) {
			if (imports.ModuleType.get(k) == "command") {
				if (
					imports.ModuleImports.get(k).CommandName.includes(
					...
					try {
						...
						if (
							imports.ModuleImports.get(k).CommandToggleWhitelist
						) {
							//Todo: deprecated whitelist function

							if (
								!imports.ModuleImports.get(
									k,
								...
								if (msg.guild) {
									await interaction.reply({
										content:
											":no_entry_sign: **Sorry,** *But you can't use that command in this channel.*",
										ephemeral: true,
									});
									return;
								}
						...
						if (interaction.guildId) {
							...
							if (!imports.ModuleImports.get(k).CommandRunGuild) {
								await interaction.reply({
									content:
										":no_entry_sign: **Sorry,** *But you can't use this command in guilds, please use dms.*",
									ephemeral: true,
								});
								return;
							}
							...
							if (!imports.ModuleImports.get(k).CommandRunDM) {
								await interaction.reply({
									content:
										":no_entry_sign: **Sorry,** *But you can't use this command in dms, please use guilds.*",
									ephemeral: true,
								});
								return;
							}
							...
							if (imports.ModuleImports.get(k).Permissions == 0) {
								...
								await interaction.reply({
									content:
										":no_entry_sign: **Sorry,** *But you don't have the permissions to run this command.*",
									ephemeral: true,
								});
							}
						}
						...
						console.log(e);
						var mvdw_fix_this_you_lazy_cuck =
							imports.bot.users.cache.get("843501785603506246");
						mvdw_fix_this_you_lazy_cuck.send(
							"Yo!! You maybe fucked up somewhere..\n\n```" +
								e +
								"```\nOk thanks.",
						);
						...
						await interaction.reply({
							content:
								"I'm terribly sorry but an error occurred during processing of the command. The developers have been informed.",
							ephemeral: true,
						});
					}
				}
