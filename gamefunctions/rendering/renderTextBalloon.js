	...
	renderTextBalloon: async function (interaction) {
		...
		if (data.enable_textballoon) {
			...
			if (data.textballoon_fadein && p_render.frame <= 10) {
				...
				p_render.ctx.fillText(
					textFormatting.textFormatting(
						data.textballoon_author,
						p_root,
					),
					199,
					172,
				...
				p_render.ctx.fillText(
					textFormatting
						.textFormatting(data.textballoon_text, p_root)
						.toString()
						.slice(0, frame_offset),
					199,
					208,
				);
			...
			if (data.textballoon_text.length == frame_offset) {
				...
				renderImage.render(
					interaction,
					"./assets/images/textdone.png",
					p_render.canvas.width - 22,
					p_render.canvas.height - 22,
				);
