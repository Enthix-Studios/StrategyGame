...
glofunc.GetAllFilesFromPath(ModualNormalizedPath).forEach(function (file) {
	...
	try {
		...
		var type_module =
			imports.ModuleImports.get(file).ModuleType.toLowerCase();
		...
		if (type_module == "command") {
			// Command metadata
			var command_module =
				imports.ModuleImports.get(file).CommandName[0].toLowerCase();
			var command_description =
				imports.ModuleImports.get(file).CommandDescription;

			// Add command to list.
			const command_builder = new SlashCommandBuilder()
				.setName(command_module)
		...
		console.log(e);
		process.stdout.write(
			"\x1b[31mERROR: The module '" +
				file +
				"' will not be loaded in, please fix the issue.\x1b[0m\n",
		);
	}
});
...
if (mods_loaded == mods_total) {
	console.log(
		"\x1b[36m[" +
			...
			"/" +
			mods_total +
			"] \x1b[32mEverything loaded up, logging into servers...\x1b[0m\n",
	);
} else {
	console.log(
		"\x1b[36m[" +
			...
			"/" +
			mods_total +
			"] \x1b[33mWARNING: Not all modules seem to be loaded! We will try to continue but some things may be broken. logging into servers...\x1b[0m\n",
	);
}
